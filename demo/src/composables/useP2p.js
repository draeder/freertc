...
export function useP2p({
  ...
  function _connectPeerOoo(modeLabel) {
    ...
    const LIVING_PEER_FRESHNESS_MS = 15000   // Treat live mesh entries as fresh for 15s
    ...
    function shouldDialCandidate(candidate) {
      ...
      if (existing && isMeshAliveState(existing.state) && existing.connection?.signalingState !== 'closed') {
        ...
        const isFresh = Date.now() - lastSeen < LIVING_PEER_FRESHNESS_MS
    ...
    function collectDeadMeshFallbackCandidates() {
      const fallback = []
      for (const [remotePeerId, entry] of _signalingClient?.mesh.connections.entries() ?? []) {
        if (!remotePeerId || remotePeerId === localPeerId) continue
        if (entry?.state !== 'dead') continue
        fallback.push({
          peerId: remotePeerId,
          lastSeenAt: entry.lastSeen ?? 0,
          source: 'mesh-dead-fallback',
        })
      }
      return fallback
    }

    function mergeDialCandidates(candidates) {
      const byPeerId = new Map()
      for (const candidate of candidates || []) {
        if (!candidate?.peerId || candidate.peerId === localPeerId) continue
        byPeerId.set(candidate.peerId, candidate)
      }
      for (const candidate of collectDeadMeshFallbackCandidates()) {
        if (!byPeerId.has(candidate.peerId)) byPeerId.set(candidate.peerId, candidate)
      }
    ...
    function maybeDialCandidates(candidates) {
      const sorted = sortCandidates(mergeDialCandidates(candidates))
      for (const candidate of sorted) {
        ...
        if (existing && isMeshAliveState(existing.state) && existing.connection?.signalingState !== 'closed') {
          ...
          const isFresh = Date.now() - lastSeen < LIVING_PEER_FRESHNESS_MS
          if (!isFresh) {
            try { existing.connection?.close() } catch { /* ignore */ }
            _signalingClient?.mesh.connections.delete(remotePeerId)
          }
        }
